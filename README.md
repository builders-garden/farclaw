...
4. Fill required provider secrets:
   ...
   - Easy option in Neynar dashboard: `Apps` -> `Your App` -> `Agents` tab -> `Use existing account` (copy signer UUID)
...
6. Set security/config variables:
   ...
   - These values are editable, but with `${secret(...)}` they are auto-generated by Railway at deploy time, so you usually do not need to change them manually.
...
9. Pair Telegram only if you did not set `TELEGRAM_ALLOWED_USER_ID`:
